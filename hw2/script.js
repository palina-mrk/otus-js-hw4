document.querySelector("#task1").onclick = () => {
  ...
  if (areNumbers(a, b)) console.log(`max (${a}, ${b}) = ${calcMax(a, b)}`);
...
document.querySelector("#task3").onclick = () => {
  ...
  if (arePositive(circle, square))
    console.log(
      `The circle with area ${circle} ${isInscribed(circle, square) ? "can" : "can't"} be inscribed into square with area ${square}`,
    );
  else console.log(`Wrong input`);
};
