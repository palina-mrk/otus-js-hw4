...
document.querySelector("#task3").onclick = () => {
  ...
  console.log(
    `${str} ${isTelephoneNumber(str) ? "is" : "is not"} a valid telephone number`,
  );
};
...
function isDate(str) {
  ...
  if (str.length != 10 || !regexp.test(str)) return false;
  ...
  if (
    ...
    !isBetween(year, 0, 3000)
  )
    return false;

  // проверяем 31 число
  if (day == 31 && !has31days(month)) return false;

  // проверяем високосность
  if (day == 29 && !isLeap(year)) return false;
...
function isTelephoneNumber(str) {
  ...
  //не-цифры считаем необязательными
  const regexp =
    /\+\s?375\s?[0-9]{2,3}\s?[0-9]{2,3}[\s\-]?[0-9]{2}[\s\-]?[0-9]{2}/;
  // строка должна содержать regexp
  // и полностью с ним совпадать
  if (!isRegexp(str, regexp)) return false;

  //если regexp - OK, проверяем код города/моб.оператора
  const digits = str
    .split("")
    .filter((el) => el === "0" || (el >= 1 && el <= 9));
