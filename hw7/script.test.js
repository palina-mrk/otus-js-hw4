...
describe(`Checks the html-elements of InputForm`, () => {
  ...
  it(`contains one hidden button`, () => {
    ...
    expect(testForm.container.getElementsByTagName("button")[0]).toBe(
      testForm.button,
    );
  ...
  it(`contains one empty input field`, () => {
    ...
    expect(testForm.container.getElementsByTagName("input")[0]).toBe(
      testForm.input,
    );
  ...
  it(`contains three non-empty paragraphs`, () => {
    ...
    for (let i = 0; i < 3; i++) {
      expect(testForm.container.getElementsByTagName("p")[i]).toBe(
        testForm.paragraphs[i],
      );
  ...
  it(`contains a title h4`, () => {
    ...
    expect(testForm.container.getElementsByTagName("h4")[0]).toBe(
      testForm.title,
    );
  });
...
describe(`Checks the methods of InputForm`, () => {
  ...
  it(`removes the first paragraph, if they exist,
    ...
    testForm.paragraphs.forEach((el, ind) => {
      el.innerText = `${testForm.paragraphs.length - 1 - ind}`;
    });
  ...
  it(`adds a paragraph with the message 
    ...
    testForm.paragraphs.forEach((el, ind) => {
      if (ind == pCount) expect(el.innerText).toBe("my text");
      else expect(el.innerText).toBe(ind + "");
    });
  });
});
...
describe("Checks the event listener onclick", () => {
  let testForm;
  {
    ...
    it(`click on button causes adding a paragraph
      ...
      testForm.paragraphs.forEach((el, i) => {
        el.innerText = `${i}`;
      });
    ...
    it(`click on button causes adding a paragraph
      ...
      for (let counter = 6; counter < 10; counter++) {
        ...
        expect(testForm.paragraphs.length).toBe(5);
        expect(
          testForm.paragraphs.find((el) => Boolean(el == textToClear)),
        ).toBe(undefined);
      }
    });
  }
